...
| 영역 | 이유 | 계획 |
|---|---|---|
| cross-origin iframe | 현재 hook은 top frame에만 주입 | frame별 host permission과 `allFrames` prototype |
...
미지원 영역은 `captureMetadata.issues` 또는 collection-level coverage summary로 노출한다. 조용히 빈 schema를 만들어 높은 confidence를 주지 않는다.

## 검증

정적 fixture는 REST, GraphQL, multipart, nested envelope, schema variation과 path identifier 오탐을 확인한다. Chromium runtime은 실제 `fetch`, XHR, GraphQL 2개 operation, FormData file upload, 등록 payload 및 캡처 종료 후 관찰 중단을 검증한다.
